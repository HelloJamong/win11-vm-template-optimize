...
if (($EnableTempCleanup -or $EnableInstallerResidueCleanup -or $EnableSetupLogCleanup -or $EnableDownloadsDesktopCleanup) -and (Confirm-Step -Title '[1/41] 임시 파일 및 캐시 정리' -Details $tempDetails)) {
    $SystemCleanupPaths = @()
...
# -----------------------------
if ($EnableUpdateCacheCleanup -and (Confirm-Step -Title '[2/41] Windows Update 캐시 정리' -Details @(
...
# Defender Cleanup
# -----------------------------
if ($EnableDefenderCleanup -and (Confirm-Step -Title '[3/41] Windows Defender 검사 기록 정리' -Details @(
...
# Event Logs Cleanup
# -----------------------------
if ($EnableEventLogClear -and (Confirm-Step -Title '[4/41] 이벤트 로그 초기화' -Details @(
...
# Hibernation Off
# -----------------------------
if ($EnableHibernationOff -and (Confirm-Step -Title '[5/41] 최대 절전 비활성화' -Details @(
    'powercfg -h off 실행',
...
# -----------------------------
if ($EnablePowerPlanTweaks -and (Confirm-Step -Title '[6/41] 전원 계획 및 절전 설정 조정' -Details @(
...
# -----------------------------
if ($EnablePagefileDisable -and (Confirm-Step -Title '[7/41] Pagefile 비활성화' -Details @(
...
# Appx Removal
# -----------------------------
if ($EnableAppxRemoval -and (Confirm-Step -Title '[8/41] Appx 앱 제거' -Details (@(
...
# -----------------------------
if ($EnableProvisionedAppxRemoval -and (Confirm-Step -Title '[9/41] Provisioned Appx 제거' -Details @(
...
# -----------------------------
if ($EnableOneDriveRemoval -and (Confirm-Step -Title '[10/41] OneDrive 제거' -Details @(
...
# -----------------------------
if ($EnableServiceOptimization -and (Confirm-Step -Title '[11/41] 서비스 비활성화' -Details (@(
...
# -----------------------------
if ($EnableScheduledTaskOptimization -and (Confirm-Step -Title '[12/41] 예약 작업 비활성화' -Details (@(
...
# -----------------------------
if ($EnableSearchTweaks -and (Confirm-Step -Title '[13/41] 검색/Bing/클라우드 연계 차단 (레지스트리)' -Details @(
    'AllowCortana = 0',
    'DisableWebSearch = 1',
...
# Copilot Tweaks
# -----------------------------
if ($EnableCopilotTweaks -and (Confirm-Step -Title '[14/41] Copilot 비활성화 (레지스트리)' -Details @(
...
# Recall Tweaks
# -----------------------------
if ($EnableRecallTweaks -and (Confirm-Step -Title '[15/41] AI/Recall 기능 차단 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableConsumerTweaks -and (Confirm-Step -Title '[16/41] 소비자 경험/광고/추천 앱 비활성화 (레지스트리)' -Details @(
...
# Privacy Tweaks
# -----------------------------
if ($EnablePrivacyTweaks -and (Confirm-Step -Title '[17/41] 개인정보/텔레메트리 정책 적용 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnablePrivacyGeneralTweak -and (Confirm-Step -Title '[18/41] 개인 정보 및 보안 > 일반/권장 사항 조정 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableSignInOptionsTweak -and (Confirm-Step -Title '[19/41] 로그인 옵션 조정 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableTaskbarEndTaskTweak -and (Confirm-Step -Title '[20/41] 작업 표시줄 작업 종료 버튼 활성화 (레지스트리)' -Details @(
...
# App Restart Tweak
# -----------------------------
if ($EnableAppRestartTweak -and (Confirm-Step -Title '[21/41] 앱 자동 재시작 비활성화 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableDeliveryOptimizationTweaks -and (Confirm-Step -Title '[22/41] Delivery Optimization 외부 공유 차단 (레지스트리)' -Details @(
...
# Explorer / UI Tweaks
# -----------------------------
if ($EnableExplorerTweaks -and (Confirm-Step -Title '[23/41] 파일 탐색기 UI 조정 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableExplorerPrivacyCleanup -and (Confirm-Step -Title '[24/41] 파일 탐색기 사용 흔적 정리 (레지스트리 + 파일)' -Details @(
...
# Start Menu Tweaks
# -----------------------------
if ($EnableStartMenuTweaks -and (Confirm-Step -Title '[25/41] 시작 메뉴 추천/최근 항목 표시 제한 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableStartPersonalizationTweak -and (Confirm-Step -Title '[26/41] 개인설정 > 시작 조정 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableTaskbarAndNotificationTweaks -and (Confirm-Step -Title '[27/41] 작업표시줄/알림 정리 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableLockScreenContentTweaks -and (Confirm-Step -Title '[28/41] 잠금화면 콘텐츠 제한 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableOptionalFeatureDisable -and (Confirm-Step -Title '[29/41] Windows 선택적 기능 비활성화' -Details @(
...
# CleanMgr
# -----------------------------
if ($EnableCleanMgr -and (Confirm-Step -Title '[30/41] Windows 디스크 정리 (cleanmgr)' -Details @(
...
if ($EnableDismCleanup -and (Confirm-Step -Title '[31/41] DISM 컴포넌트 저장소 정리' -Details $dismDetails)) {
...
# CompactOS (optional)
# -----------------------------
if ($EnableCompactOS -and (Confirm-Step -Title '[32/41] CompactOS 적용' -Details @(
...
# Edge Tweaks
# -----------------------------
if ($EnableEdgeTweaks -and (Confirm-Step -Title '[33/41] Microsoft Edge 최적화 (VDOT 기준 정책 레지스트리)' -Details @(
...
# -----------------------------
if ($EnableControlPanelViewTweak -and (Confirm-Step -Title '[34/41] 제어판 보기 기준: 큰 아이콘' -Details @(
...
# Boot Timeout Tweak
# -----------------------------
if ($EnableBootTimeoutTweak -and (Confirm-Step -Title '[35/41] 시작 및 복구: OS 목록 표시 시간 3초' -Details @(
    'bcdedit /timeout 3'
))) {
...
# System Volume Tweak
# -----------------------------
if ($EnableSystemVolumeTweak -and (Confirm-Step -Title '[36/41] 시스템 볼륨 50% 설정' -Details @(
...
# Computer Rename
# -----------------------------
if ($EnableComputerRename -and (Confirm-Step -Title "[37/41] 컴퓨터 이름 변경: $NewComputerName" -Details @(
...
# -----------------------------
if ($EnableVisualEffectsTweak -and (Confirm-Step -Title '[38/41] 성능 옵션 시각 효과: Custom (4개만 활성화)' -Details @(
...
# Desktop Icons
# -----------------------------
if ($EnableDesktopIcons -and (Confirm-Step -Title '[39/41] 바탕화면 시스템 아이콘 표시: 내 PC / 제어판' -Details @(
...
# -----------------------------
if ($EnableStartMenuPinnedCleanup -and (Confirm-Step -Title '[40/41] 시작 메뉴 고정 항목 정리 (Edge / 파일 탐색기 / 설정만 유지)' -Details @(
...
# Defrag Free Space
# -----------------------------
if ($EnableDefragFreeSpace -and (Confirm-Step -Title '[41/41] 여유 공간 통합 (defrag /X)' -Details @(
