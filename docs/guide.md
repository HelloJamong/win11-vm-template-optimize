...
```text
VM-Optimize.zip
├─ win11_master_template_optimize.ps1   ← 설치 완료 후 최적화 스크립트
...
프로그램 설치 및 설정이 완료된 후 **현재 로그인된 계정**으로 관리자 PowerShell을 열어 실행합니다.
HKCU 레지스트리 설정이 실행 계정에 적용되므로 반드시 실제 사용 계정으로 로그인한 상태에서 실행해야 합니다.
...
# 비대화형 (모든 단계 자동 진행)
.\win11_master_template_optimize.ps1 -NonInteractive
```

### 실행 전 확인 항목

스크립트 상단 Options 섹션에서 아래 항목을 먼저 확인합니다.
