...
param(
    ...
    # -NonInteractive 지정 시 모든 단계를 사용자 확인 없이 자동 진행합니다.
)
...
# -NonInteractive 스위치가 지정되면 false, 기본(대화형)은 true입니다.

# 각 함수 내부에서 -ErrorAction SilentlyContinue를 개별 지정하므로 전역 설정이 불필요합니다.
# 전역 SilentlyContinue는 의도치 않은 오류까지 무음 처리해 문제 추적을 어렵게 만들어 제거했습니다.
...
$EnableNetBiosDisable              = $true
# NetBIOS over TCP/IP를 비활성화합니다.
# 공공기관/망분리 환경에서 불필요한 NetBIOS 브로드캐스트 제거 및 보안 강화 목적입니다.
# WINS 또는 NetBIOS 기반 이름 해석이 필요한 환경에서는 false로 변경하십시오.

$EnableNetBiosFirewallBlock        = $true
# NetBIOS 관련 포트 137/138/139 인바운드를 방화벽으로 차단합니다.
# 12단계 NetBIOS 비활성화와 함께 이중 차단을 구성합니다.

$EnableSmbFirewallBlock            = $true
# SMB 포트 445/TCP 인바운드를 방화벽으로 차단합니다.
# 완전 망분리·독립 VM 환경에서 권장합니다.
# 도메인 가입 또는 조직 파일 서버 연결이 필요한 환경에서는 false로 변경하십시오.
...
$NewComputerName                   = 'VDI-Win11'
# 컴퓨터 이름을 $NewComputerName으로 변경합니다. Sysprep 전 적용 권장. (재부팅 필요)
        ...
        try {
            Get-ChildItem -LiteralPath $Path -Force -ErrorAction SilentlyContinue | ForEach-Object {
                try {
                    ...
                    Write-Log "항목 삭제 실패: $($_.FullName) — $_" 'WARN'
                }
            }
        }
        catch {
            Write-Log "경로 내용 조회/정리 실패: $Path — $_" 'WARN'
        }
    }
}
...
function Remove-PathIfExists {
    ...
    if (Test-Path -LiteralPath $Path) {
        ...
        try {
            ...
            Write-Log "경로 삭제 실패: $Path — $_" 'WARN'
        }
    }
}
# 지정 파일/폴더 자체를 제거합니다.
...
function Stop-ServiceSafe {
    ...
    try {
        ...
        Write-Log "서비스 중지 실패: $Name — $_" 'WARN'
    }
}
...
function Start-ServiceSafe {
    ...
    try {
        ...
        Write-Log "서비스 시작 실패: $Name — $_" 'WARN'
    }
}
# 정리 후 필요 서비스 재시작용입니다.

function Invoke-WithStoppedCleanupServices {
    param(
    ...
    if (-not $StopServices) {
        try { & $Action }
        catch { Write-Log "정리 단계 실행 실패: $_" 'WARN' }
        return
    }
    ...
    try {
        ...
        Write-Log "캐시 정리 단계 실행 실패: $_" 'WARN'
    }
    finally {
        if ($servicesTouched) {
...
# Windows Update/BITS/Delivery Optimization 캐시 정리 중 파일 잠금을 줄이되,
# 중간 실패/중단 시에도 finally에서 서비스 재시작을 시도합니다.
...
function Disable-ServiceSafe {
    ...
    try {
        ...
        Write-Log "서비스 비활성화 실패: $Name — $_" 'WARN'
    }
}
...
function Disable-TaskByPathAndName {
    ...
    try {
        ...
        Write-Log "예약 작업 비활성화 실패: $TaskPath$TaskName — $_" 'WARN'
    }
}
# 예약 작업을 비활성화합니다.
...
# 캐시/다운로드 파일 잠금을 줄이기 위한 서비스 중지는 실제 정리 단계 진입 후
# Invoke-WithStoppedCleanupServices에서 수행합니다.
...
if (($EnableTempCleanup -or $EnableInstallerResidueCleanup -or $EnableSetupLogCleanup -or $EnableDownloadsDesktopCleanup) -and (Confirm-Step -Title '[1/44] 임시 파일 및 캐시 정리' -Details $tempDetails)) {
    Invoke-WithStoppedCleanupServices -StopServices:$EnableTempCleanup -Action {
    $SystemCleanupPaths = @()
...
# -----------------------------
if ($EnableUpdateCacheCleanup -and (Confirm-Step -Title '[2/44] Windows Update 캐시 정리' -Details @(
    'SoftwareDistribution\Download 삭제',
    'SoftwareDistribution\DeliveryOptimization 삭제'
))) {
    Invoke-WithStoppedCleanupServices -Action {
...
# Defender Cleanup
# -----------------------------
if ($EnableDefenderCleanup -and (Confirm-Step -Title '[3/44] Windows Defender 검사 기록 정리' -Details @(
    'Windows Defender\Scans\History 삭제',
    'Windows Defender\Scans\Tmp 삭제'
))) {
    Remove-ChildrenIfExists "$env:ProgramData\Microsoft\Windows Defender\Scans\History"
    Remove-ChildrenIfExists "$env:ProgramData\Microsoft\Windows Defender\Scans\Tmp"
}
...
# Event Logs Cleanup
# -----------------------------
if ($EnableEventLogClear -and (Confirm-Step -Title '[4/44] 이벤트 로그 초기화' -Details @(
    ...
    try {
        wevtutil el | ForEach-Object {
            try { wevtutil cl "$_" } catch { Write-Log "이벤트 로그 초기화 실패: $_" 'WARN' }
        }
    }
    catch {
        Write-Log "이벤트 로그 목록 조회 실패: $_" 'WARN'
    }
}
...
# Hibernation Off
# -----------------------------
if ($EnableHibernationOff -and (Confirm-Step -Title '[5/44] 최대 절전 비활성화' -Details @(
    'powercfg -h off 실행',
    'hiberfil.sys 제거 (수 GB 용량 확보)'
))) {
    Write-Log 'Disabling hibernation'
    try { powercfg -h off | Out-Null } catch { Write-Log "최대 절전 비활성화 실패: $_" 'WARN' }
}
# hiberfil.sys 제거
...
# -----------------------------
if ($EnablePowerPlanTweaks -and (Confirm-Step -Title '[6/44] 전원 계획 및 절전 설정 조정' -Details @(
...
# -----------------------------
if ($EnablePagefileDisable -and (Confirm-Step -Title '[7/44] Pagefile 비활성화' -Details @(
    '자동 pagefile 관리 해제 (Win32_ComputerSystem AutomaticManagedPagefile = false)',
    'pagefile 설정 제거 시도 (재부팅 후 C:\pagefile.sys 제거/검증 필요)',
    ...
    try {
        ...
        Write-Log "Pagefile 설정 제거 실패: $_" 'WARN'
    }
}
# pagefile 자동 관리 해제 + pagefile 설정 제거 시도
# 재부팅/검증이 필요할 수 있어 기본 false

# -----------------------------
# Appx Removal
# -----------------------------
if ($EnableAppxRemoval -and (Confirm-Step -Title '[8/44] Appx 앱 제거' -Details (@(
    ...
    foreach ($pattern in $AppxPatterns) {
        try {
            ...
            Write-Log "Appx 제거 패턴 처리 실패: $pattern — $_" 'WARN'
        }
    }
}
...
# -----------------------------
if ($EnableProvisionedAppxRemoval -and (Confirm-Step -Title '[9/44] Provisioned Appx 제거' -Details @(
    ...
    try {
        ...
        Write-Log "Provisioned Appx 제거 처리 실패: $_" 'WARN'
    }
}
...
# -----------------------------
if ($EnableOneDriveRemoval -and (Confirm-Step -Title '[10/44] OneDrive 제거' -Details @(
    ...
    try {
        Start-Process "$env:SystemRoot\System32\OneDriveSetup.exe" '/uninstall' -Wait -NoNewWindow -ErrorAction Stop
    }
    catch {
        Write-Log "OneDrive 64-bit 제거 실행 실패: $_" 'WARN'
    }
    try {
        Start-Process "$env:SystemRoot\SysWOW64\OneDriveSetup.exe" '/uninstall' -Wait -NoNewWindow -ErrorAction Stop
    }
    catch {
        Write-Log "OneDrive 32-bit 제거 실행 실패: $_" 'WARN'
    }
...
# -----------------------------
if ($EnableServiceOptimization -and (Confirm-Step -Title '[11/44] 서비스 비활성화' -Details (@(
...
# NetBIOS Disable
# -----------------------------
if ($EnableNetBiosDisable -and (Confirm-Step -Title '[12/44] NetBIOS over TCP/IP 비활성화' -Details @(
    'IP 활성 어댑터 전체에 SetTCPIPNetBIOS(2) 적용',
    '2 = Disable NetBIOS over TCP/IP',
    '경고: WINS 또는 NetBIOS 기반 이름 해석이 필요한 환경에서는 건너뛰십시오'
))) {
    Write-Log 'Disabling NetBIOS over TCP/IP on all adapters'
    try {
        Get-CimInstance -ClassName Win32_NetworkAdapterConfiguration -Filter 'IPEnabled=True' |
            ForEach-Object {
                $result = Invoke-CimMethod -InputObject $_ -MethodName SetTCPIPNetBIOS -Arguments @{ TcpipNetbiosOptions = [uint32]2 }
                if ($result.ReturnValue -eq 0) {
                    Write-Log "NetBIOS disabled: $($_.Description)"
                } else {
                    Write-Log "NetBIOS disable skipped/failed: $($_.Description) (code $($result.ReturnValue))" 'WARN'
                }
            }
    }
    catch {
        Write-Log "NetBIOS 비활성화 실패: $_" 'WARN'
    }
}
# NetBIOS over TCP/IP 비활성화로 불필요한 브로드캐스트를 줄이고 LLMNR/NetBIOS 기반 MITM 공격 노출을 줄입니다.

# -----------------------------
# NetBIOS Firewall Block (137/138/139)
# -----------------------------
if ($EnableNetBiosFirewallBlock -and (Confirm-Step -Title '[13/44] NetBIOS 포트 인바운드 차단 (137/138/139)' -Details @(
    '137/UDP (NetBIOS Name Service) 인바운드 차단',
    '138/UDP (NetBIOS Datagram Service) 인바운드 차단',
    '139/TCP (NetBIOS Session Service) 인바운드 차단',
    'Windows Defender 방화벽 인바운드 규칙 추가 (모든 프로필)',
    '12단계 NetBIOS 비활성화와 함께 이중 차단 구성'
))) {
    Write-Log 'Adding inbound firewall block rules for NetBIOS ports 137/138/139'
    $netbiosFirewallRules = @(
        @{ Name = 'Block-NetBIOS-NBNS-137-UDP-In';     Protocol = 'UDP'; Port = 137; Desc = 'NetBIOS Name Service 인바운드 차단' },
        @{ Name = 'Block-NetBIOS-Datagram-138-UDP-In'; Protocol = 'UDP'; Port = 138; Desc = 'NetBIOS Datagram Service 인바운드 차단' },
        @{ Name = 'Block-NetBIOS-Session-139-TCP-In';  Protocol = 'TCP'; Port = 139; Desc = 'NetBIOS Session Service 인바운드 차단' }
    )
    foreach ($rule in $netbiosFirewallRules) {
        try {
            if (-not (Get-NetFirewallRule -Name $rule.Name -ErrorAction SilentlyContinue)) {
                ...
                Write-Log "Firewall rule added: $($rule.Name) ($($rule.Protocol)/$($rule.Port) inbound block)"
            } else {
                Write-Log "Firewall rule already exists, skipped: $($rule.Name)" 'WARN'
            }
        }
        catch {
            Write-Log "Firewall rule failed: $($rule.Name) — $_" 'WARN'
        }
    }
}
# NetBIOS 포트 방화벽 이중 차단으로 서비스 비활성화 우회 경로를 추가 차단합니다.

# -----------------------------
# SMB Firewall Block (445)
# -----------------------------
if ($EnableSmbFirewallBlock -and (Confirm-Step -Title '[14/44] SMB 포트 인바운드 차단 (445/TCP)' -Details @(
    '445/TCP (SMB Direct) 인바운드 차단',
    'Windows Defender 방화벽 인바운드 규칙 추가 (모든 프로필)',
    '경고: 도메인 가입 환경에서는 GP/SYSVOL 접근 및 네트워크 드라이브 연결이 차단됩니다',
    '경고: 완전 망분리·독립 VM 환경에서만 적용을 권장합니다'
))) {
    Write-Log 'Adding inbound firewall block rule for SMB port 445' 'WARN'
    try {
        $smbRuleName = 'Block-SMB-445-TCP-In'
        if (-not (Get-NetFirewallRule -Name $smbRuleName -ErrorAction SilentlyContinue)) {
            New-NetFirewallRule -Name $smbRuleName -DisplayName 'SMB Direct 인바운드 차단' `
                -Direction Inbound -Protocol TCP -LocalPort 445 `
                -Action Block -Enabled True -Profile Any | Out-Null
            Write-Log "Firewall rule added: $smbRuleName (TCP/445 inbound block)"
        } else {
            Write-Log "Firewall rule already exists, skipped: $smbRuleName" 'WARN'
        }
    }
    catch {
        Write-Log "Firewall rule failed: Block-SMB-445-TCP-In — $_" 'WARN'
    }
}
# 445 인바운드 차단으로 WannaCry/NotPetya 류 SMB 기반 전파 경로를 차단합니다.
# 도메인·파일서버 연결이 필요한 환경은 이 단계를 건너뛰십시오.

# -----------------------------
# Scheduled Task Optimization
# -----------------------------
if ($EnableScheduledTaskOptimization -and (Confirm-Step -Title '[15/44] 예약 작업 비활성화' -Details (@(
...
# -----------------------------
if ($EnableSearchTweaks -and (Confirm-Step -Title '[16/44] 검색/Bing/클라우드 연계 차단 (레지스트리)' -Details @(
    'AllowCortana = 0',
    'DisableWebSearch = 1',
...
# Copilot Tweaks
# -----------------------------
if ($EnableCopilotTweaks -and (Confirm-Step -Title '[17/44] Copilot 비활성화 (레지스트리)' -Details @(
...
# Recall Tweaks
# -----------------------------
if ($EnableRecallTweaks -and (Confirm-Step -Title '[18/44] AI/Recall 기능 차단 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableConsumerTweaks -and (Confirm-Step -Title '[19/44] 소비자 경험/광고/추천 앱 비활성화 (레지스트리)' -Details @(
...
# Privacy Tweaks
# -----------------------------
if ($EnablePrivacyTweaks -and (Confirm-Step -Title '[20/44] 개인정보/텔레메트리 정책 적용 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnablePrivacyGeneralTweak -and (Confirm-Step -Title '[21/44] 개인 정보 및 보안 > 일반/권장 사항 조정 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableSignInOptionsTweak -and (Confirm-Step -Title '[22/44] 로그인 옵션 조정 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableTaskbarEndTaskTweak -and (Confirm-Step -Title '[23/44] 작업 표시줄 작업 종료 버튼 활성화 (레지스트리)' -Details @(
...
# App Restart Tweak
# -----------------------------
if ($EnableAppRestartTweak -and (Confirm-Step -Title '[24/44] 앱 자동 재시작 비활성화 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableDeliveryOptimizationTweaks -and (Confirm-Step -Title '[25/44] Delivery Optimization 외부 공유 차단 (레지스트리)' -Details @(
    'Windows 업데이트 > 고급 옵션 > 다운로드 최적화 > 다른 장치에서 다운로드 허용 = 끔',
...
# 다운로드 최적화의 다른 장치 다운로드 허용을 끄고 불필요한 네트워크 공유/트래픽을 억제합니다.

# -----------------------------
# Explorer / UI Tweaks
# -----------------------------
if ($EnableExplorerTweaks -and (Confirm-Step -Title '[26/44] 파일 탐색기 UI 조정 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableExplorerPrivacyCleanup -and (Confirm-Step -Title '[27/44] 파일 탐색기 사용 흔적 정리 (레지스트리 + 파일)' -Details @(
...
# Start Menu Tweaks
# -----------------------------
if ($EnableStartMenuTweaks -and (Confirm-Step -Title '[28/44] 시작 메뉴 추천/최근 항목 표시 제한 (레지스트리)' -Details @(
    'ShowRecentList = 0, ShowFrequentList = 0, ShowRecommendations = 0',
    'Start_TrackDocs = 0, Start_TrackProgs = 0'
    # SubscribedContent-338388/338389 는 소비자 경험 섹션에서 통합 처리
))) {
    ...
    # SubscribedContent-338388/338389 는 EnableConsumerTweaks 에서 단일 관리
}
...
# -----------------------------
if ($EnableStartPersonalizationTweak -and (Confirm-Step -Title '[29/44] 개인설정 > 시작 조정 (레지스트리)' -Details @(
...
# -----------------------------
if ($EnableTaskbarAndNotificationTweaks -and (Confirm-Step -Title '[30/44] 작업표시줄/알림 정리 (레지스트리)' -Details @(
    'ShowTaskViewButton = 0 (작업 보기 버튼 숨김)',
    'TaskbarDa = 0 (Widgets 숨김)',
    'AllowNewsAndInterests = 0',
    'ContentDeliveryManager 310093, 338393 비활성화',
    # SubscribedContent-353694/353696 는 소비자 경험 섹션에서 통합 처리
    ...
    # SubscribedContent-353694/353696 는 EnableConsumerTweaks 에서 단일 관리
...
# -----------------------------
if ($EnableLockScreenContentTweaks -and (Confirm-Step -Title '[31/44] 잠금화면 콘텐츠 제한 (레지스트리)' -Details @(
    'RotatingLockScreenEnabled = 0 (Spotlight 비활성화)',
    'RotatingLockScreenOverlayEnabled = 0',
    'SubscribedContent-338387Enabled = 0',
    'LockScreenOverlayEnabled = 0',
    # SoftLandingEnabled 는 소비자 경험 섹션에서 통합 처리
    ...
    # SoftLandingEnabled 는 EnableConsumerTweaks 에서 단일 관리
...
# -----------------------------
if ($EnableOptionalFeatureDisable -and (Confirm-Step -Title '[32/44] Windows 선택적 기능 비활성화' -Details @(
    ...
    foreach ($f in $Features) {
        try {
            ...
            Write-Log "선택적 기능 비활성화 실패: $f — $_" 'WARN'
        }
    }
}
...
# CleanMgr
# -----------------------------
if ($EnableCleanMgr -and (Confirm-Step -Title '[33/44] Windows 디스크 정리 (cleanmgr)' -Details @(
    'cleanmgr.exe /verylowdisk 실행 (완료까지 대기)',
    '시스템 기본 디스크 정리 항목 자동 처리'
))) {
    Write-Log 'Running cleanmgr'
    try {
        Start-Process cleanmgr.exe '/verylowdisk' -Wait -NoNewWindow -ErrorAction Stop
    }
    catch {
        Write-Log "cleanmgr 실행 실패: $_" 'WARN'
    }
}
# Windows 기본 디스크 정리
...
if ($EnableDismCleanup -and (Confirm-Step -Title '[34/44] DISM 컴포넌트 저장소 정리' -Details $dismDetails)) {
    ...
    try {
        ...
        Start-Process dism.exe -ArgumentList $dismArgs -Wait -NoNewWindow -ErrorAction Stop
    }
    catch {
        Write-Log "DISM 정리 실행 실패: $_" 'WARN'
    }
}
# 컴포넌트 저장소 정리의 핵심
...
# CompactOS (optional)
# -----------------------------
if ($EnableCompactOS -and (Confirm-Step -Title '[35/44] CompactOS 적용' -Details @(
    ...
    try {
        Start-Process compact.exe '/compactos:always' -Wait -NoNewWindow -ErrorAction Stop
    }
    catch {
        Write-Log "CompactOS 실행 실패: $_" 'WARN'
    }
}
...
# Edge Tweaks
# -----------------------------
if ($EnableEdgeTweaks -and (Confirm-Step -Title '[36/44] Microsoft Edge 최적화 (VDOT 기준 정책 레지스트리)' -Details @(
...
# -----------------------------
if ($EnableControlPanelViewTweak -and (Confirm-Step -Title '[37/44] 제어판 보기 기준: 큰 아이콘' -Details @(
...
# Boot Timeout Tweak
# -----------------------------
if ($EnableBootTimeoutTweak -and (Confirm-Step -Title '[38/44] 시작 및 복구: OS 목록 표시 시간 3초' -Details @(
    'bcdedit /timeout 3'
))) {
    Write-Log 'Setting boot timeout to 3 seconds'
    try {
        Start-Process bcdedit.exe -ArgumentList '/timeout', '3' -Wait -NoNewWindow -ErrorAction Stop
...
# System Volume Tweak
# -----------------------------
if ($EnableSystemVolumeTweak -and (Confirm-Step -Title '[39/44] 시스템 볼륨 50% 설정' -Details @(
...
# Computer Rename
# -----------------------------
if ($EnableComputerRename -and (Confirm-Step -Title "[40/44] 컴퓨터 이름 변경: $NewComputerName" -Details @(
    ...
    try {
        ...
        Write-Log "Computer renamed to $NewComputerName (재부팅 후 적용)"
    } catch {
...
# -----------------------------
if ($EnableVisualEffectsTweak -and (Confirm-Step -Title '[41/44] 성능 옵션 시각 효과: Custom (4개만 활성화)' -Details @(
...
# Desktop Icons
# -----------------------------
if ($EnableDesktopIcons -and (Confirm-Step -Title '[42/44] 바탕화면 시스템 아이콘 표시: 내 PC / 제어판' -Details @(
...
# -----------------------------
if ($EnableStartMenuPinnedCleanup -and (Confirm-Step -Title '[43/44] 시작 메뉴 고정 항목 정리 (Edge / 파일 탐색기 / 설정만 유지)' -Details @(
  ...
  "pinnedList": [
    ...
    $defaultLayoutDir = "$env:SystemDrive\Users\Default\AppData\Local\Microsoft\Windows\Shell"
    try {
...
# 서비스 재시작은 각 캐시 정리 단계의 Invoke-WithStoppedCleanupServices finally 블록에서 수행합니다.

# -----------------------------
# Defrag Free Space
# -----------------------------
if ($EnableDefragFreeSpace -and (Confirm-Step -Title '[44/44] 여유 공간 통합 (defrag /X)' -Details @(
    ...
    try {
        Start-Process defrag.exe -ArgumentList $env:SystemDrive, '/X', '/U', '/V' -Wait -NoNewWindow -ErrorAction Stop
    }
    catch {
        Write-Log "defrag 실행 실패: $_" 'WARN'
    }
}
